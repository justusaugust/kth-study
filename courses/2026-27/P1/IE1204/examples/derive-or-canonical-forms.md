...
Combine the last two terms: $A\overline{B}+AB=A$. Then $F=\overline{A}B+A=A+B$ by covering. The only zero-row is $00$, whose maxterm is already $A+B$, so canonical POS reaches the same expression immediately.
